 ...
 * material, 588 triangles, 32 m tall.
 *
 ...
 * not the ruins' #3A2B24): fourteen dodecahedra and five plates. The
 * dodecahedra are three's own at radius 1 under a scale triple the
 * generator baked into the buffer (seabed.mjs `dodeca`) — a base mound
 * 7.36 by 2.53 by 6.21 and a spur beside it, six lobes stacked from 4 m to
 * 27.8 m and shrinking as they climb, a crown at 30.2 m stretched tall, and
 * four flat shelves let into the flanks between 21 and 27.4 m — each on a
 * node that carries a small tilt and its own yaw. Between the lobes sit
 * five plates, three's six-facet cylinder at 0.72 of its base radius on
 * top, from 5.4 m across at the bottom to 2.7 m at the top, kept exactly as
 * three built them: indexed, smooth-shaded, the torso's normals 21–24°
 * apart (seabed.mjs `kept`), which `parts.mjs` prints without the
 * "non-indexed" it prints for everything else here.
 *
 * Two of the file's Euler triples print flipped — `lobe_2` as
 * (−3.02, 1.54, −3.06), which is (0.12, 1.6, 0.08) — and the script writes
 * the plain form, since the quaternion, and so the matrix the file carries,
 * is the same.
 *
 ...
 * carries that one factor and nothing else (kit.mjs `fitFootprint`; not
 * seabed.mjs `stand`, whose lift the file does not carry).
 * `diff.mjs env-coral-tower HEAD` divides the factor out and lists nothing
 * else.
 */
import { THREE, add, cyl, fitFootprint, exportGlb } from '../kit.mjs';
...
const size = fitFootprint(tower, FOOTPRINT);
const drawn = Math.max(size.x, size.z);
if (Math.abs(drawn - DRAWN) > 1e-3)
  throw new Error(`env_coral_tower: drawn ${drawn.toFixed(4)} across; the header says ${DRAWN}`);
console.log(
  `env_coral_tower: drawn ${drawn.toFixed(4)} across, held at ${FOOTPRINT} m (×${tower.scale.x.toFixed(5)})`
);
