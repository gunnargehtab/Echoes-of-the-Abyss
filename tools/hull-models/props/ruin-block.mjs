 ...
 * did: every axis-aligned coordinate and nineteen in twenty of the rest
 * reproduce to the float32 bit, the remainder within one ulp. What the
 * factor is a ratio of was not recovered; it is neither a footprint fit
 * nor a height fit against any extent this design has. Then the generator
 ...
 * root carries that one factor and no lift (seabed.mjs `held`).
 * `diff.mjs env-ruin-block HEAD` divides it out and lists nothing else.
 */
...
const CORAL = [
  ...
  ['coral_03', octa, [1.68, 0.72, 1.44], [6.5, 7, -7.4], [0.2, 0.4, 0.1]],
  ['coral_04', seabed.ico, [1.65, 0.75, 2.1], [-8.7, 7.1, 6.8], [0.5, 0.1, 0.2]],
  ['coral_05', octa, [0.78, 0.91, 2.08], [8.6, 6.9, -2], [0, 0.6, 0.3]],
  ['coral_06', seabed.ico, [1.69, 0.715, 1.43], [-6.7, 13, -5.7], [0.2, 0.9, 0.1]],
  ['coral_07', octa, [1.32, 0.77, 1.32], [5.6, 13, -5.4], [0.4, 0.2, 0]],
  ['coral_08', seabed.ico, [1.2, 0.6, 1.68], [-6.8, 13.1, 4.4], [0.1, 0.4, 0.5]],
  ['coral_09', octa, [1.5, 0.6, 1], [0.6, 13, 4.6], [0.3, 0.1, 0.2]],
  ['coral_10', seabed.ico, [1.68, 0.84, 1.68], [-4.6, 19.1, -3.6], [0.6, 0.3, 0.1]],
  ['coral_11', octa, [1.43, 0.88, 1.21], [2.6, 19, 2.5], [0.2, 0.7, 0.4]],
  ['coral_12', seabed.ico, [1.6, 0.45, 1], [-1, 19.1, 2.7], [0.1, 0.1, 0.1]],
  ['coral_13', octa, [1.54, 1.26, 1.54], [2.9, 7.6, 3.6], [0.4, 0.6, 0.2]],
  ['coral_14', seabed.ico, [1.68, 0.6, 1.44], [6.1, 0.9, 5.4], [0, 0.3, 0.6]],
  ['coral_15', octa, [1, 1, 1.3], [7.9, 1.7, 3.6], [0.5, 0, 0.3]],
  ['coral_16', octa, [0.48, 1.44, 1.8], [9, 3.4, -5], [0, 0.2, 0]],
  ['coral_17', octa, [0.44, 1.43, 1.43], [-9, 4.2, -1.5], [0, 0.1, 0.3]],
];
for (const [name, shape, size, at, tilt] of CORAL)
  add(block, name, seabed.kept(shape === octa ? octa(1) : shape()), coral, at, tilt, size);
...
const { drawn, k } = seabed.held(block, FOOTPRINT, { drawn: DRAWN });
console.log(
