 ...
 * at z 6.0 and y 0.5 (rule 5). Not at the ring's own station, nor at the
 * approved disc's: a flat disc of r 1.35 would reach z 8.05 centred on
 * the ring (6.7) and 7.97 at the disc's own 6.62, past both files' bow
 * extents by three's `Box3`, which `metreTrue` measures — 7.6594 on the
 * Directorate's, `tergite_port_3`'s box (the mandibles reach 7.5035),
 * and 7.3904 on the Commune's, `root_anchor_1`'s — and `metreTrue`
 * refuses a drawn length that is not the header's
 ...
 * forge line; the bakes' gains went from ×0.074 to ×0.056 and from ×0.069
 * to ×0.052. Nothing else moved. Both files that call this pass the same
 * role, so the one decision holds for both.
 */
