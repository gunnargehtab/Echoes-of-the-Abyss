 ...
 *   own plan station, its centre line sunk 0.01 under the skin read
 *   straight down off the lobes and growth rings beneath it, rolled and
 *   pitched to that skin (`veinRing` `on`). The file laid each ring flat
 *   at one height on its lobe's waist: the core ring at 0.42, where the
 *   crown's skin is 0.62–0.68, so it rides the crown now, rolled 23–25°
 *   to the crown's fall; core-2 at 0.18, where the bed under its 1.75
 *   radius is the west lobe's crown at 0.59–0.45, then the cleft between
 *   the west and north lobes, where the core's shoulder shows and the
 *   segments follow it down to 0.36 and 0.26 rolled 59–60° onto its
 *   flank, the fifth across `growth-ring-3`'s tube where that pokes out,
 *   then the north lobe's crown at 0.37–0.44; the west ring at 0.3 and
 *   the east at 0.32, each of whose first two segments come down the
 *   core's flank at 0.60 and 0.47 (0.58 and 0.47), pitched 21–24°, onto
 *   their own lobe's crown at 0.38–0.42 (0.41–0.45). Names, count,
 ...
 * another part (#890 review rulings, ruling 1), lit as the file lights
 * it, in place. Clearing it would shorten or turn the ring's arc, a shape
 * decision this change does not make; the second segment shows its outer
 * end past the mound. The other thirty show 1.9–16.4 m² each, two of them
 * (`west` 1 and 5) shaded by stalk-4's pod and stalk-6's stem, which lean
 * over the west ring where the file grew them.
 */
