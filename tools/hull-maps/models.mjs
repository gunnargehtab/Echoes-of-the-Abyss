/**
 * The model table: which approved model clads which unit or structure, and
 * the design size it is read against. Data only, because two build steps
 * read it — build.mjs bakes the maps from it and outlines.mjs takes each
 * kind's plan outline from it — and a table that lived in one of them would
 * have to be restated in the other.
 *
 * A kind absent here keeps its procedural fallback (units: the
 * distance-transform bake and a hand-drawn HULL_OUTLINE; structures: the
 * slab-and-landmark bake). That is the intended state for hulls whose model
...
export const STRUCTURES = [
  ...
  // one structure whose SIG gap is the tell — 25 idle against 50 rendering
  // (systems-flora.md §2 and §7), so a reactor gone quiet has stripped its
  // bed. The rows bake at the idle 25, as the Foundry's do against its own
  // 25/55, because the table's figure is the idle one. 180 m is 2 × radiusM 90.
