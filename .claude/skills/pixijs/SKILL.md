...
> **Only five of the twenty-six PixiJS skills are vendored here:** `pixijs`
> (this router), `pixijs-scene-graphics`, `pixijs-scene-text`,
> `pixijs-performance` and `pixijs-ticker`. Those are the surfaces this
> repository's HUD actually uses. **Every other row in the router below, and
> every entry in `references/index.md`, names a skill that is not present on
> disk** — do not try to read it. For those topics go straight to the
> `llms.txt` fallback at the foot of this file.
