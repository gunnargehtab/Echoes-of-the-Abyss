...
| Invariants (`/spec/invariants.md`) | Was scattered — stated as an invariant in prose in one place, and asserted across the suites | **Yes** — `docs/invariants.md`, 31 rows over 67 holders (`npm run check:invariants` at `1d02acc`), each naming its source and the test that holds it, with that gate failing when a holder is gone |
