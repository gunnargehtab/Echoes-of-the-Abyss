...
| Cross-mission Drift Health — rule 5's second pair | **Built** (#379), and the pair is real on both sides now: [mission-in-writing.md](mission-in-writing.md) reuses `anholt-furrow` unchanged, and the record keeps the furrow's grid per map id, so what this day's dive did to the ground is what that one starts from — less one flat recovery, `DRIFT.CARRY_RECOVERY`, capped at the biome's start. Nothing tells the player; the furrow is the message |
