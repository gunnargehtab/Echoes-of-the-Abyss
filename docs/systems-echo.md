...
- **Detection radius renders as a soft ring on the terrain** — for a selected hull, and for any hull loud enough to be worth warning about. A player must be able to see their own reach without first clicking the hull that has it. Drawn as **one envelope over the fleet** rather than a circle per hull: an arc appears only where it bounds water no other own hull already hears into, which is the same field with its redundant interior lines left out ([ui-ux.md](ui-ux.md) §3.5).
