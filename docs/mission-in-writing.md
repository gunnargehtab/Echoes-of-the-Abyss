...
map down on its first stationary pass at about 05:04, at 13.3 hull a second per gun against
...
at 300 hull against 80 every 2.7 s has eleven seconds. At 15:00 the guns are ordered back to the
...
| **Bloom Surge** — the Commune's superweapon, and the row the format brief owes this title | **Absent, and deliberately not asked for.** [factions.md](factions.md) described a detonating spore mass that blinds hydrophones across a vast radius for 25 s, and now (#438) a seeded Veil that ripens for 20 s at SIG 45 and caps every hydrophone in 1,200 m at Tier 1 for 15 s, both sides alike; nothing in `packages/` carries either, and no commander-ability or superweapon layer exists ([mission-convocation.md](mission-convocation.md) §13). This mission's system is the Veil as a *structure* — a bed that stays, under which hiding is a schedule — and a burst that blinds the dome for fifteen seconds would be an answer to the letter in the letter's own grammar, which the Commune declines to give. The mission is playable without it and would be worse with it. `inWriting.ts` authors no `commanderAbility` and the test asserts the absence, so the refusal is in the literal and not only in this document |
...
| **Cross-mission Drift Health** — rule 5's second concrete pair | **Built** (#379), against the first pair [mission-convocation.md](mission-convocation.md) §13 named, and this one asks nothing further of it. A furrow the last mission dived through loudly is a quieter garden this tide, and nobody tells the player why. Per-map Drift Health is the second of the three sibling keys `store.ts` was shaped for and is now written and read; because it is keyed by map rather than by mission, reusing `anholt-furrow` unchanged is the whole of what this document had to do |
