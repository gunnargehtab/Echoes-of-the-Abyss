...
the plateau can hear again. Drift Health carried from *Deep Furrow* (rule 5) is built (#379),
keyed by map rather than by mission, so this pair carries it exactly as *Tend*'s does (§13).
...
| **Cross-mission Drift Health** — rule 5's second concrete pair | **Built** (#379), by the same mechanism as the first and with no authoring of its own: the carry is keyed by **map id**, not by mission, so a rule built against [mission-convocation.md](mission-convocation.md) §13's pair holds for this one because *Deep Furrow* and this tide resolve to one `anholt-furrow`. The grid the first closed on is the grid the second seeds from, every living cell healed by one flat `DRIFT.CARRY_RECOVERY` (5) across the three tides between them and a Dead cell left Dead. A furrow the last mission dived through loudly is a quieter garden this tide, and nobody tells the player why: the record keeps the grid per map, the client presents it when it creates the room, the server bounds it and seeds it, and no briefing, mark or panel reads it. The second of the three sibling keys `store.ts` was shaped for is now written |
