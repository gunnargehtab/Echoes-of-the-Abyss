...
| Cross-mission Drift Health (§2 rule 5) — the second system standing on that record | **Built** (#379), against the first pair the bible authored on purpose — [Tend](mission-tend.md) and [Convocation](mission-convocation.md) on one `marr-plateau` — and the second it authored since, [Deep Furrow](mission-deep-furrow.md) and [In Writing](mission-in-writing.md) on one `anholt-furrow`. The close hands the client the map's final region grid on the `missionOver` payload, public for the reason `EchoSnapshot.driftHealth` already is (bestiary.md §5 — the tell is light, not sound), and the record keeps it **per map id** as a third sibling key beside `missions` and `scenes`: the one field in the record that is *replaced* on every close rather than kept at its best, because a map that healed is not a better reading, it is a later one. The client presents the grid when it creates the next mission on that map, and the room bounds it — exact length, every cell a finite reading on §6's 0–100 scale, the whole grid refused for one bad cell rather than clamped — before `DriftHealth.seed` writes it in ahead of the herds. The gap between missions is one flat recovery, `DRIFT.CARRY_RECOVERY` (TUNABLE, 5 points), capped at the biome's start so a carried map is never healthier than a fresh one; a dead cell stays dead, as §6's table says it does. The record lives in the client's storage, so the server can hold a grid to what is *possible* and not to what is *true*; the cap is what stops the carry being a lever upward, and a cleared browser is a fresh map, which rule 5 permits. Nothing tells the player. The map is the message |
