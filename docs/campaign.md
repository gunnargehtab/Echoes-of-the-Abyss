...
| **The other three campaigns** — 21 missions | **First missions specified and built** ([mission-tend.md](mission-tend.md), [mission-attendance.md](mission-attendance.md), [mission-aptitude.md](mission-aptitude.md)) — Aptitude last, and with it every mission the bible had specified had a literal behind it. **[Thin Water](mission-thin-water.md) was the first document past that line and is now built as well** — `kell-shoulder` and `seeding-thin-water`, with tests — and its §13's prediction held: the escort hold, `souls`, the emitter window and the condition-fired beat all already shipped, and the one thing the literal wanted that the document had not anticipated was a second terminal row, because the outcome ladder is read off how many terminal objectives were met and its §8 Results table has three rows. **[Intake](mission-intake.md) and [Standing Wave](mission-standing-wave.md) are the queue behind it: specified and not built.** Intake broke that streak — its §13 found the first thing a specified mission had wanted and not had since the Ledger's three additions landed, a predicate over Biomass, the union's `deliver` being Nodules only, though the income behind it already shipped; that predicate is now built (#330), `deliver` generalised over the economy record's three accounts, and Intake's literal is the only thing its §13 still waits on. Standing Wave asks for a great deal more: its §13 is the first that is a design agenda rather than a build list, headed by a docs-versus-code disagreement it deliberately does not settle — the corridor's PF 2.0 against `MAX_PROPAGATION_FACTOR`, which is 1.60 and derived — plus a predicate over what the player has built, which the objective union has never carried. The other 15 rows of §5–§7 remain titles, teaching targets and beats, and each needs a document of its own before it needs code |
