...
| "Already seen" briefing variants (§1) — the first of the three systems standing on that record | **Built** (#378): a *filed* [Tend](mission-tend.md) changes the briefings for [Thin Water](mission-thin-water.md) and [Convocation](mission-convocation.md). [Baffle](mission-baffle.md) and [The Dome](mission-the-dome.md) also recognise their common closure announcement in either play order: the 04:00 line earns a side-specific stamp when spoken, and each header reads only the other side's stamp. A completed mission without that hearing selects the default; a lost mission that heard it carries the stamp. The **seen-scene set** remains beside `missions`, with no migration or new persistence channel. Alternates ship in public headers and are selected client-side: the room never knows which briefing was read. No in-mission line, order, timing or outcome changes |
...
- **[campaign-read.md](campaign-read.md)** — this campaign read in play order rather than side by side: what a player knows at three checkpoints, what the two cross-campaign scenes tell them twice in the same words, and eight findings about how the story arrives
