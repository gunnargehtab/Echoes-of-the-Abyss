...
| **The other three campaigns** — 21 missions | **First missions specified and built** ([mission-tend.md](mission-tend.md), [mission-attendance.md](mission-attendance.md), [mission-aptitude.md](mission-aptitude.md)) — Aptitude last, and with it every mission the bible had specified had a literal behind it. **[Thin Water](mission-thin-water.md) is the first document past that line: specified and not built**, the first of the 18, and its §13 reports that the format asks for nothing new — the escort hold, `souls`, the emitter window and the condition-fired beat all already ship. **[Standing Wave](mission-standing-wave.md) is the second, and it asks for a great deal**: its §13 is the first §13 that is a design agenda rather than a build list, headed by a docs-versus-code disagreement it deliberately does not settle — the corridor's PF 2.0 against `MAX_PROPAGATION_FACTOR`, which is 1.60 and derived — plus a predicate over what the player has built, which the objective union has never carried. The other 16 rows of §5–§7 remain titles, teaching targets and beats, and each needs a document of its own before it needs code |
