...
| **The other three campaigns** — 21 missions | **Twenty of the twenty-one are built, and the one that is not is [Standing Wave](mission-standing-wave.md).** The Commune's seven are built whole — [Tend](mission-tend.md), [Thin Water](mission-thin-water.md), [Convocation](mission-convocation.md), [Deep Furrow](mission-deep-furrow.md), [In Writing](mission-in-writing.md), [Radicals](mission-radicals.md), [The Second Seeding](mission-second-seeding.md) — and so are the Directorate's — [Attendance](mission-attendance.md), [Intake](mission-intake.md), [The Dome](mission-the-dome.md), [Shallow](mission-shallow.md), [Trench Awakening](mission-trench-awakening.md), [Conclave](mission-conclave-attending.md), [First Arrival](mission-first-arrival.md). The Order has six of seven: [Aptitude](mission-aptitude.md), [Nineteen](mission-nineteen.md), [Conclave](mission-conclave-chord.md), [The Three](mission-the-three.md), [The Rim Deposits](mission-rim-deposits.md) and [The Second Chord](mission-second-chord.md), with Standing Wave the hole in the middle of its campaign. Each mission is a literal, a map and a test file that plays the tide rather than reading the literal, and each document's §13 is now an account of what its own transcription found. **The format was asked for three things by the fourteen, all of them in #391**, which is a smaller bill than the first four documents ran up between them: `MissionRegion.pressureBonus`, a rectangle of water that rates every hull standing in it over its own PR; the same field on the `ground` beat, so a grant can be turned on at a tick without repainting the water; and `holdsMovement` honouring a hull named by a `MissionHold` whether or not it is escorting anything, which was a latent bug rather than a feature — *The Second Seeding* recorded a hold that the runtime had never enforced, because only a tender with a populated `lastEscorted` could be held. The grant resolves as **max and not sum** in `auras.ts`, so a hull standing under both a Sounding Spire and a granted region takes the larger of the two. **What the fourteen mostly found was arithmetic, and the documents were mostly the side that moved** — a walk measured to an approximation that had been deleted, a strip table measured to the nearer hull where the mission is decided by the further, three figures carried over from a working depth that had since been corrected, five animals where `spawnFauna` makes one, a tier label a range table contradicts, and in one case an inference that inverted its own paragraph's conclusion. Where a document and a constant disagreed the constant won, because constants live in exactly one place; where a document and a *literal* disagreed, the document was read first and the literal fixed. Two findings ran the other way and changed code: the speaker labels §12 gives Adze, and a scope drawing Drift Health against a bare 75 where the simulation reads `DRIFT.HEALTH_STRAINED`. **What Standing Wave still wants is what it wanted before** — a predicate over what the player has *built*, which the objective union has never carried — and its §13's other rows; the two design calls beside it were made in #372, the corridor keeping PF 2.0 behind a ceiling the map reports about itself, and the build radius going to the pairing range of 1,500 m for every structure |
