...
Be honest about the reach this buys. Twenty of the twenty-three maps paint a full-extent background region that sets a floor, so the rule is trivially true everywhere on them; of the three left it binds for real on the two that paint only what they mean, and the third is the exemption below. What it buys on the twenty is that a background has to be a statement about the water column rather than a biome wash. A map-level `floorM` does not satisfy it, and that is not an oversight: every map sets one, so counting it would make the rule vacuously true everywhere — on the fixture above too, which is the rule switched off rather than widened.

**One map is exempt, and the exemption is the Kelp Labyrinth's.** It states its water column once, at the map level — `floorM: 1800`, with its reason beside it — and shapes no region near a spawn; its outer Coral Ruins ring is a biome and nothing else. All eight of its placements fail the rule and not one is a finding: they open on ground the map shaped, stated another way. The exemption is the whole map rather than its corners, because the corners are not special — every point on this map is in the same position. It is an exemption from the strengthening and not from the rule: the map is still held to containment, so it keeps everything the older reading bought. The alternative was to restate `floorM: 1800` on the background region, which paints no cell differently and is a line written to satisfy a guard — the fault the guard is for. The map tests refuse an exemption that has stopped being needed, so this one cannot outlive its reason. Every other spawn and Foundry in `sim/maps/` satisfies the rule, mission maps included, and the map tests refuse any that does not.
...
| The Western Margin | `first-trench-margin` | #212 | 5,000 × 3,000 m. The trench country's top step, six sounds of somebody else's economy, and the layer as a door — [mission-exposure.md](mission-exposure.md) §11 |
| The Underworks | `holding-underworks` | #212 | 4,000 × 3,000 m. The Holding's wall, the roofed works beneath it, and a choice authored as terrain — [mission-tolerance.md](mission-tolerance.md) §11 |
| The Rim | `mouth-rim` | #212 | 6,000 × 4,000 m. The Mouth's edge: staging, slopes, terraces and the lip, entirely below the layer — [mission-prospect.md](mission-prospect.md) §11 |
