...
| Cross-mission Drift Health, and the roster | **Not built** ([campaign.md](campaign.md) §2 rule 5), and the absence is confirmed rather than closed now that both neighbours exist. A row that kills its own cells here should find them dead the next time this map is played, and nothing carries a map's damage forward — the one campaign row where the absence is a lost lesson rather than a lost convenience, and it is not asked for here. Nor does anything carry a roster: [mission-first-arrival.md](mission-first-arrival.md)'s literal now stands (#397) and authors its twelve Choristers fresh, from its own party table, so the cohort grown in this yard is that column **in prose only** and a row that loses eight hulls here walks the lip with twelve all the same. `MissionRecord` was shaped to take mission-scoped fields without a migration and named these among the three queued behind it; the first of the three has since landed — the seen-scene set that selects a briefing variant (#378, shipped in #395) — which is the proof that the shape works and leaves these two as fields nobody has written yet |
...
| In-mission character speech, heard | Text only, the standing status ([mission-sorrowgate.md](mission-sorrowgate.md) §13). The ten scheduled lines land at §9's ticks and in §9's order and the two tally lines fire off the account rather than the clock, and every one of them is read |
