...
| Cross-mission Drift Health | **Not built** — nothing carries a map's damage to the next mission on it, which is [campaign.md](campaign.md) §2 rule 5 unspent everywhere. It is one of the three sibling keys `Progression` was shaped to receive (`packages/frontend/src/progression/store.ts`), beside the seen-scene set that has since landed and a spent roster that has not, and nothing writes it. §11 states what this mission would leave behind and reads no cell — and the row two above corrects *which side* leaves it |
