...
is **83**, which wears the ground at 0.006 a second ([bestiary.md](bestiary.md) §6). The one
...
### If the closure was heard from the convoy

The default above is read unless the record holds `fourth-closure-convoy`: the
Picket-Speaker's **04:00** line below, heard while playing [Baffle](mission-baffle.md).
Completing that mission without hearing the line does not qualify. Replace only the
second paragraph of the whole briefing, Korrin's first, with:

> The Fourth is closed while the exchange inquiry is open, and has been closed for three
> tides. The closure has been heard from the convoy's side: what enters is being counted,
> not threatened. A relief convoy is at the north staging under a writ that has not been
> filed. It will enter the trench. The count is entered from this side now; the convoy's
> need and the closure are not entered as answers to one another.

The same line here earns `fourth-closure-picket` when its `say` beat fires. Neither arrival,
the sweep filing nor the mission outcome substitutes for hearing it. The close carries
the stamp even on a lost watch; replays add no duplicate. Each header reads only the
other side's stamp. The alternate names no unplayed consequence, no outcome of the Call,
and nothing in the deep water. Every other paragraph, all in-mission speech, and all
mission mechanics remain unchanged.
