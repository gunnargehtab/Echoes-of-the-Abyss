...
| `AiCommand` carries 21 verbs against the 27 a seated commander can send, and the parity claim in `ai/types.ts` is checked only in the direction that cannot fail — so "the AI plays through the interface a player plays through" is decoration wherever the two sets differ | [#621](https://github.com/gunnargehtab/Echoes-of-the-Abyss/issues/621) |
