...
| Flora wave 3 — the bio-reactor, the first thing in the game that can take crop: 400 m, 12 Biomass a minute at SIG 50, eating its own cover nearest-first so the canopy over it falls as it runs and a mature reactor sits in open water, loudly, in the hole it made. A full bed is 240 Biomass and therefore a match's work, which makes a bed a claim to defend rather than a tap to sit on. The health it costs a region is derived from the seeding roster rather than chosen, and the stored baseline must *not* move — the map it runs on authors no kelp | [#557](https://github.com/gunnargehtab/Echoes-of-the-Abyss/issues/557) |
