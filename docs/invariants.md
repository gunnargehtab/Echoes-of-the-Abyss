...
| 16 | A hull the commander claims as a tender is left in a state bloom-share is actually paid for — loud, and in the Shelf band — because claiming it removes it from every other branch that would have ordered either | [economy.md](economy.md) §6 — bloom-share is the Commune's income, and `bloomShare.ts` pays only a tender that is in the band and not silent; #706 | `packages/backend/test/aiFlora.test.ts` — `lifts Silent Running off the hull it sends gardening`; `packages/backend/test/aiFlora.test.ts` — `brings a tender hanging under the rim up into the Shelf band` |
