...
| 5 | Every spawn and its Foundry stands inside an authored region — on every map, mission maps included | [maps.md](maps.md), "How a map is written"; #622 | `packages/backend/test/maps.test.ts` — `paints the ground under every base it seats, on every map` |
...
| 14 | The relief shade only ever darkens an authored fill, scales with the size of the ground step rather than saturating, and moves luminance without moving hue | [graphics-standards.md](graphics-standards.md); [style-neon-noir.md](style-neon-noir.md) — the shading is a lighting model, checked against the light rather than a screenshot | `packages/frontend/test/palette.test.ts` — `reliefShade` |
