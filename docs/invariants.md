...
| 16 | The shell sends no order for a hull the mission is holding, and reads a selection as held only when **every** hull in it is — so a mixed selection still goes | [ui-ux.md](ui-ux.md) §10.5 — the lock is continuous state, learned before the press, and an order to a mixed selection still moves what is not held; #478, #708 | `packages/frontend/test/movementHolds.test.ts` — `a selection the mission holds whole says so before the press`; `packages/frontend/test/rendererSmoke.test.ts` — `sends no order at all for a hull the mission is holding` |
| 17 | Every order that moves a hull asks the mission's hold first — move, attack-move, embark, dive, follow-floor and harvest alike | [mission-sorrowgate.md](mission-sorrowgate.md) §8; #478, #708 — the clamp runs at 5 Hz and movement at 60, so a path that skips the refusal is not merely late, it is unbounded | `packages/backend/test/missionHold.test.ts` — `a harvest order is a movement order, and the hold refuses it too` |
