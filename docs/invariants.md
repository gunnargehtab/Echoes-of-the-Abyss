...
| 16 | A movement affordance is dead only for a selection the mission is holding **whole** — a mixed selection keeps every one of them, because the order still moves what is not held | [ui-ux.md](ui-ux.md) §10.5 — the lock is continuous state, learned before the press, and a mixed selection still goes; #478, #708 | `packages/frontend/test/movementHolds.test.ts` — `a selection the mission holds whole says so before the press` |
