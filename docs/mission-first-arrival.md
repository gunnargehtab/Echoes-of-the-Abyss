...
| The map, reused unchanged | **Decided, and not a build.** `mouth-rim` is `packages/backend/src/sim/maps/missions/mouthRim.ts`, authored for [mission-prospect.md](mission-prospect.md) §11; §11's table is that literal's table, row for row, and the literal for this mission adds no region and moves no metre. Cross-mission Drift Health on a shared map — [campaign.md](campaign.md) §2 rule 5 — is **built** (#379) and keyed by map, so this mission inherits the chart *and* what D's two tides did to the ground: a rim surveyed loudly on D is a rim with a ledger on D+1, and nothing tells the player so |
