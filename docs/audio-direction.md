...
| Arbiter Mosk Halloran | the court | a heavier tap, high-passed at 900 Hz against the register's 1,200, read at 2.8 — the count, said aloud and left to sit. Spent on the prologue's four opening lines (#726) |
...
**Built** (#381): the five hails, the murmur bed and its reading rule, the `speech` bus and its trim, the rung and its table, the whisper rule, and the register on the wire — `MissionVoice` in `@echoes/shared`, `voice` on the `say` beat and on `MissionLine`, authored on the minority of lines spoken by somebody other than the player's own faction and defaulted to the player's register otherwise. Sorrowgate's four voices in the water are the first content, and they are four registers by construction — the Order at 06:20, the concern at 09:00, the cohorts at 09:20, the plateaus at 10:40 — and `missionRuntime.test.ts` plays the mission and asserts they arrive as four different voices. **The court's hail carries content since #726**: Halloran reads four lines into the same mission's opening window, at 00:20, 01:00, 01:50 and 02:40 ([mission-sorrowgate.md](mission-sorrowgate.md) §9), so the fifth register is no longer built-and-unspent. The briefing and the objective readings are still read before the socket opens and are still not beats.
