...
**A cohort hull is good for exactly one strike.** That sentence is the mission's whole account of
...
killed — 9,000 HP against an intake's 44.4/s each — it cannot be reasoned with, and the
Directorate's own superweapon summons one and does not steer it ([factions.md](factions.md)).
...
| --- | --- | --- |
| *The band is answered* — 245 Biomass rendered | Biomass banked ≥ 245 (§13 — the one predicate this mission needs and the format lacks) | Yes |
...
having searched well and is the only reward in the mission.
...
[mission-asset-recovery.md](mission-asset-recovery.md) §13 started. **This document is a
specification and nothing in it is built** — [campaign.md](campaign.md) §11's "other three
campaigns" row is where that stands. The table below is what the mission would need on the day
somebody writes its literal, and unlike [mission-thin-water.md](mission-thin-water.md)'s it does
**not** answer "nothing new": this is the mission that finds out the format cannot price an
economy that is not Nodules.
...
| **The finding, as a choice** | **Built** — a `choiceGroup` over two conditional beats, the mechanism [mission-tolerance.md](mission-tolerance.md) §6 asked for and #282 landed. One fires on `extract` into `the-ascent`, one on the whole intake mustering, and whichever fires retires the other. Nothing new is needed, and the reason is worth stating: **the roll deliberately never asks which hull.** A predicate that named one would need a hull to carry both a counted role and a named one, and `MissionUnit.role` is singular — see the row below |
...
| **Biomass as an objective — the predicate this mission needs and the format lacks** | **Not built.** The eight predicates are `extract`, `survive`, `attend`, `sound`, `quiet`, `endure`, `tolerance` and `deliver`, and `deliver` is Nodules only — its own comment calls it "the union's first economic row", written against [mission-shift-change.md](mission-shift-change.md) §8. **The income itself already ships**, which is the correction this document makes to its own issue: `world.ts` carries `economy = { nodules, crystal, biomass }`, `payBiomass` credits a fauna kill to the nearest owner at full rate for the Directorate and `DRIFT.RENDERING_CONTRACT_RATE` (0.3) for everyone else, and `match.ts` already reports biomass per slot. Only the *query* is missing. **The shape this document wants** is a generalisation of `deliver` over the economy record's own three accounts — `{ kind: 'deliver'; account: 'nodules' \| 'crystal' \| 'biomass'; amount: number }` — rather than a `biomass` sibling, because a sibling would be the second of three near-identical rows and the third is already visible in the Knights' campaign. Note it must key on the **economy record**, not on `ResourceKind`: that enum names field nodes (Nodule, ResonanceCrystal) and Biomass has no node — it is paid on a kill |
| **Biomass as something to spend** | **Not built**, and the other half of §6 row 2. Biomass accrues and nothing consumes it: `UnitStats` prices hulls in `cost` (Nodules) and `crystalCost`, and no production path reads the biomass account. This mission does not need it — the intake is twelve hulls and there is nothing to build (§2) — but *"cheap expendable units"* cannot be taught by any mission until spending exists, and mission 5 is the one that will ask |
| **A cheap cohort hull** | **Not in the roster**, and this is a docs-to-docs disagreement rather than a missing mechanism, so it is flagged rather than decided. [economy.md](economy.md) §6 says the Directorate is "cheapest per unit" and that "Directorate cohorts are inexpensive"; [units.md](units.md) prices their only hull at **260 Nodules and 80 Resonance Crystal**, against a Corvette's 120 and a Light Scout's 50 — the second most expensive hull in the game and the only crystal-locked one. Both cannot be true. Either the roster needs a cheap cohort entry beneath the Submersible, or `economy.md` §6's sentence means *cheapest per point of value* and should say so. **That is a design call and not an unattended one**; this document fields the Submersible because it is what exists, and §4's arithmetic is honest about what twelve of them cost |
...
| **The Sounder as the mission's threat** | **Built, and aimed at exactly this faction by accident.** `DRIFT.TRANSIT_MIN_HULL_M` is 95 and the Abyssal Submersible's `hullLengthM` is 95, so the strict `<` test means the Directorate's only hull is the shortest one a colossus grinds — a Corvette at 80 and a Light Scout at 60 are ignored. Nothing was added for this. The bite path returns early for every non-structure, so a Sounder that *stops* is harmless; it is the swept transit that kills, which is why §6 authors a line across the map rather than an engagement |
| **The Hollows, placed and not driven** | **Built, with one seam.** `MissionBeatEffect`'s `creature` row carries a required `driveTo`, so an ambusher that must not be driven is authored with `driveTo` at its own spawn and left to its trigger model. That works and is the idiom this document assumes; it is also the one place the format shows that it was written for creatures that arrive |
...
| The map, its eight regions, the overhangs that barely lift | **Not built** — `banding-ground` would be one row of the literal per row of §11's table. No new region shape, no new biome, no hazard sites, no resource nodes |
| The mission definition `attending-intake` | **Not built** — a separate row of #212, per this document's own issue. Everything it needs exists except the Biomass predicate above |
...
- **[mission-shift-change.md](mission-shift-change.md)** — the other economy mission, and the `deliver` predicate this one cannot use
