...
table also carries two findings the literal makes against the engine as built — the colossus and
the region ledger — in the manner of §3's note, and settles neither.
...
| **The colossus, killed** | **A finding, and a design call this document does not make.** [bestiary.md](bestiary.md) §4 rates a Sounder at 9,000 HP and 260 Biomass and says it "cannot be reliably killed by any single player before the twenty-minute mark"; §6 above says "it cannot be killed — 9,000 HP against an intake's 44.4/s each". Twelve of those are 533 a second. Guns auto-acquire anything with a finite commit inside 650 m, the transit is inside 650 m of the muster for some forty seconds, and an intake that never moved and never went quiet brings the colossus down before it reaches the line — seventeen seconds of fire — and is paid the roster's 260 for it, which is the band. That is the roster disagreeing with the bestiary's own sentence, not this literal's to retune, and the literal does not: the transit is authored as §6 writes it, a year that goes quiet at the call meets it exactly as §6 describes, and `missionIntake.test.ts` states the kill so the day it is settled is noticed. Three things would change it and all three are somebody's decision: hit points that hold against a formation rather than a hull; a transit that guns cannot bring down, which [bestiary.md](bestiary.md) §4's "cannot be reliably killed" already half-says; or accepting it, which would make §6's teeth a choice the player is warned about at 15:00 and paid for declining |
| **The region ledger under a rendering** | **A second finding, stated rather than settled.** §10 says Drift Health is mission 5's system and this mission never reads it out — true, and the ledger prices the mission anyway. Three hulls cruising together are 84 of SIG against `DRIFT.HEALTH_SIG_THRESHOLD`'s 60, so they wear the cell they cross; a kill takes `HEALTH_PER_KILL` more; and a rendering in a cell under `HEALTH_STRAINED` pays three quarters — 26.25, not 35. Measured, seven renderings by a team that works the walls in order bank between 227 and 245, and the band is answered from the eighth: **the eighth is slack only for a year that spreads its work across the cells, and the ground recovers at 0.06 a second.** Twelve hulls *idling* at the muster are 264 in one cell, which strips it to nothing inside the first minute, so a colossus that dies over the muster pays nothing at all. Neither number reaches the player, per §10, and this document does not move the band; it records that "seven of eight" is the roster's arithmetic and not always the ledger's |
