...
**A hull, and its cutters running** — both halves of that sentence are conditions, and a canopy opens only for a hull that meets them. A structure is not a cutter, so a bio-reactor standing in the bed it renders does not also burn it; and **Silent Running turns the cutters off**, which is the clause [systems-echo.md](systems-echo.md) §6 applies to every worked act in this game — mining, sowing, a bloom-share tend. Silence and a thermal cutter are the same decision asked twice: a Consortium hull on silent watch in kelp takes no crop, holds no canopy open, and pays none of the noise of cutting. It is also the only way a Consortium hull is ever quiet in kelp, which is what makes "a hull sitting still in kelp is silent, and hidden" a promise to every navy rather than three.
