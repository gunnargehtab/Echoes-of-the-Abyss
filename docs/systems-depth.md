...
| Map floor (§1) | **Implemented** | Per-region, authored in the map. `DEPTH.MAX_M` is the ruleset's ceiling on what may be *ordered* — `Match.applyDepth` refuses a deeper order — and the default fill for ground a map does not paint; a map may author a floor below it, and that is water no hull holds, which the Ninth's channel at 4,100 m is (#421; [world-map.md](world-map.md) §3; [mission-attendance.md](mission-attendance.md) §11) |
