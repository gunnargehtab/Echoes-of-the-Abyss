...
| **[models-plan.md](models-plan.md)** | Phase 4 of #540: the nineteen unmodelled hulls and the Bio-Reactor, built as scripts — the touch list, the one-glow-factor rule, and each hull's builders |
...
- **The rung, and two hulls a navy** — [units.md](units.md) designs a second yard
  ...
  (#507), with a screen laid from a magazine and a shot that cannot be decoyed, and the four
  siege hulls as wave 4 (#508), with damage that reads its target class, and the two line
  hulls and the Commune's anchor as wave 5 (#509), which is where every navy stopped opening
  in a hull nobody owns.
