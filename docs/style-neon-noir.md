...
| Token | Hex | Use |
| --- | --- | --- |
| `survey-ink` | `#7A8C99` | Every isobath and coastline, blended in encoded space at 8–9.5% |
...
lifts the ground less than the quietest map-furniture outline and an unselected detection ring
do, and [map-visuals.md](map-visuals.md) §5 is the ladder that holds it there.
...
above is an aperture rather than livery, and a hull carries one, the Dredge's
fed throat aside.)
...
the ladder in [map-visuals.md](map-visuals.md) §5. An animal is an agent only as
a contact, on rung 7 (§8 there). Either way its colour comes from the palette
tables above.
