...
| **Tetherjelly** | 1,200 m; 250 m where a map re-homes it | ±100 m; ±50 m | never — does not commit | 1,100–1,300 m — the duct itself, the boundary it is named for; 200–300 m — the Kelp Forest canopy, on a map that names it |
...
**The band is where the Drift lives, not where a beat may send it.** Both sentences above are about the Drift — the creatures the seeder places and the water they pursue within — and a creature a mission *drives* is a different object, as the transit paragraph already says of its hit points. A mission may place or drive a creature below its band: a Sounder lifting off the Rim's Lip at 3,050 m ([mission-prospect.md](mission-prospect.md) §11, inherited to the metre by every rim mission), two more crossing the Ninth's head at 2,800 ([mission-conclave-attending.md](mission-conclave-attending.md) §11), a Draymaw pack driven up a trench axis at 1,600 ([mission-baffle.md](mission-baffle.md) §5; [mission-exposure.md](mission-exposure.md) §5). The seeder never will, nothing a player wakes will pursue past its band, and nothing *lives* below 2,700 m — so the table stays literally true of every animal the Drift owns. What a mission sends there is the mission's business, or the Mouth's, and the runtime treats it so: a driven creature holds the depth and the line it was given for the length of its commitment, and when the commitment lapses it climbs home to its working depth at 12 m/s, which is the seeded animal's rule taking over exactly where the beat's ends. The rim's riser is not an animal that lives on the Lip. It is one the Mouth's week sent there (#420).
...
**One species, two waters.** §4 gives the Tetherjelly two homes, "Kelp Forest, thermocline boundaries", and a species carries one working depth. This table used to choose the thermocline and flag the rest; the flag is settled (#480), and settled as **one animal re-homed per map** rather than a second species. The argument is acoustic. A canopy cluster is the same SIG 1 and the same −0.10 PF as a duct cluster: nothing about it *sounds* different, so a second name would buy the player nothing at the Echo Layer and would be a species invented to dodge a table. What differs is depth, and depth on a plateau is a property of the ground — Marr Plateau's own literal says of itself that biome is acoustics, not band. So the duct stays the species' working depth, the Kelp Forest band is **250 m ±50 m**, and a map opts into it by naming the band in its `ambientBands` ([maps.md](maps.md), "How a map is written"): every cluster on that map — seeded, placed by a beat, or released from one — rests there instead of the duct. Nothing chooses it on a map's behalf, and the seeder never places an ambient animal outside a band this table names, which is what keeps the table literally true of every animal the Drift owns on every shipped map.

Two consequences, stated because each is a choice. The shallow-masking lever is per map, not per biome: Kelp Forest ground deeper than the duct — the Kelp Labyrinth's maze floor is 1,800 m — already holds the duct population over kelp and gains nothing by re-homing, so the band exists for kelp *shallower* than 1,100 m, which on the shipped maps is Marr Plateau alone ([mission-tend.md](mission-tend.md) §11). And a canopy cluster shares the Shelf with the Lampfry, which is the water where the Commune farms the one and reads the other — the two ambient species are co-located exactly where the doctrine they serve lives.
...
**How fast a region wears is a match-length question.** Health falls where a region's summed SIG is over 60 and heals where it is under, and the two rates are deliberately asymmetric: a loud base takes its own ground from Healthy through Strained and into Failing over roughly two thirds of a match, while empty water needs about half an hour to climb the same distance back. Wearing is faster than healing, and both are slow enough to be read. What this must never be is a *timer*: measured before it was calibrated, a spawn region carried a summed SIG of 280 against a threshold of 60 and died — permanently, to zero — **twenty seconds into the match**, before the first hull was built, in all four corners of the map. Every inhabited region on every map was dead by the ten-minute mark, which is why no Biomass had ever been earned in a skirmish by anybody (#520): yield at zero health is zero.

**A creature the map kills is not a creature somebody harvested**, and the two do not cost the region the same. §8's guard-rail is that Drift Health "degrades from things players already want to do" — a commander stripping the ground it is standing on — and an eruption boiling an Ashgrazer is nobody doing anything. The region still loses the creature, and the loss still counts against it, but at a fraction of a rendering: a fauna death nobody rendered costs a quarter of what a rendered one does. Measured before this rule existed, on *Ventfront Divide*: ten of the fourteen creatures that died in a twelve-minute match were killed by the map's own eruption plumes, at the full harvesting rate, and Drift Health fell from 88 to 16 — through Failing, so nothing respawned, and into Collapsing, so what little was rendered paid a quarter. A guard-rail against over-harvesting had become a clock that ran whether or not anybody harvested.
...
| Sounder | Megafauna | **Implemented** — answers pings, destroys structures by transit, ignores small units, and takes no weapon damage while a mission beat drives it (#349). Seeded by a walk of the admissible ground rather than by rejection draws, so the one row here is one animal on any map with water deep enough to hold it; twelve draws against the 7.4% of the Ventfront Divide it may stand on used to leave a third of matches with none (#578). Migration along fixed corridors is still unwritten ([hazards.md](hazards.md) §6) |
...
| Tetherjelly | Ambient | **Implemented** — clusters seed across the duct band, or across the Kelp Forest band on a map that re-homes them (§4, "One species, two waters"; #480), and subtract 0.10 from local PF in 250 m (the hazard hook grew an additive mode; see §4). Killed clusters never return, and Failing water withers them, so PF genuinely rises toward baseline. A cluster a mission *places* masks from the tick it is born — it did not always: nothing rebuilt the PF grid for a birth until #480 |
...
§6's "sustained high SIG" is read as a **total**: every tick, `Match.driftTick` sums the raw SIG of everything a player owns in each cell of a 4 × 4 grid over the map, and the cell drains at 0.00025 a second for every point the sum stands over 60, or — when the sum is at or under it — recovers at 0.02 a second, and loses 4 for each kill somebody rendered in it and a quarter of that for one the map made (§6). The drain is the figure #520 recalibrated: it had never been set against a *sum*, and at the 0.02 written here a spawn region carrying 280 lost 4.4 health a second and was dead before the first hull was built. A cell wears or heals in a tick, never both, so the threshold is the 60 written here rather than 60 plus whatever recovery would have cancelled; and a cell at 0 stays there for the match, as the table says. The recovery rate is what "far more slowly than a match lasts" means as a number: from Dead, Failing is about 42 minutes away and Healthy more than an hour, against the 25-minute ceiling of [campaign.md](campaign.md) §10, and a kill's 4 is back in about three and a half minutes. Until #365 it was 0.06 a second, applied under the drain, and a cell stripped at the start of a long mission was Healthy before it ended — which left [campaign.md](campaign.md) §2's rule 5 nothing to carry. Fauna are left out, or the Drift would eat itself. Because it is a sum, a formation wears a cell that no one of its hulls would — three Abyssal Submersibles cruising are 84 — and a base holds its own region down for as long as it runs, which is the consequence `drift.test.ts` pins exactly rather than directionally. The first mission that consequence priced was [mission-intake.md](mission-intake.md), whose §13 records the decision to leave the ledger as it is and say so in the document (#350), and carries the figures the slower recovery moved.
