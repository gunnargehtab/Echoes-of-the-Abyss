...
differs from the committed GLB, its material's values and its normals included, so a
faction module cannot be edited without the models it moves being re-run and committed with
it. It also reads the committed files as a set and fails on a material name that carries
two values inside one navy — [asset-prompts-3d.md](asset-prompts-3d.md) Block 2b's "one
name, one value", which `node tools/hull-models/finishes.mjs` lists on its own. And since
that tool places a model by its `-<navy>.glb` suffix, the check fails on a model file whose
name places it in no navy and is not an `env-` prop (a prop belongs to none), which the
tool would otherwise skip unread. Each navy's module holds one `ink` table for the same
reason, so re-finishing a fleet is one edit (#888).
