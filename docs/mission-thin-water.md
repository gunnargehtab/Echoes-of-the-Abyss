...
| **The exposure the challenge reads, in a live match** | **Not reached** — and this is a runtime gap rather than a gap in this document. A mission seats only the player's slot, so the Echo Layer never resolves *for* a scripted party, and `EchoSnapshot.exposure` therefore stays at Tier 0 however close the corridor gets. The same holds every `tolerance` conditional in [mission-aptitude.md](mission-aptitude.md) and [mission-tolerance.md](mission-tolerance.md) shut, so it predates this mission by three documents; it is filed as its own issue (#323) and this literal is written against the condition the format specifies |
