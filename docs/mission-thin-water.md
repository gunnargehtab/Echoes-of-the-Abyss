...
somebody writes its literal, and its point is that the answer is almost entirely *nothing new*.
...
| **The challenge, fired by being heard** | **Built** — a conditional beat on `{ kind: 'tolerance'; ticks; tier }`, the exposure tally of [mission-aptitude.md](mission-aptitude.md) §5 and the condition-fired beat of its §13. One caveat, stated rather than discovered: `tolerance` reads `EchoSnapshot.exposure`, which is *any* listener's hearing of the player, so the literal is honest only because the corridor's escort is the only listener authored on this map. A mission that wanted "heard **by this party**" would be asking for a predicate that does not exist and, per the union's own information-safety note, probably should not |
...
| The map, its seven regions, trench paint on the slope and a vent under the corridor | **Not built** — `kell-shoulder` would be one row of the literal per row of §11's table. No new region shape, no new biome, no hazard sites |
| The mission definition `seeding-thin-water` | **Not built** — a separate row of #212, per this document's own issue. Everything it needs exists |
