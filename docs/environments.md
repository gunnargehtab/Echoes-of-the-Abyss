...
- **The only biome that changes state during a match** — specified here, built, and spent by three shipped missions: `attending-conclave` brings a dome down into Abyssal Trench, and `seeding-deep-furrow` and `seeding-in-writing` turn a sown furrow to Kelp Forest ([mission-conclave-attending.md](mission-conclave-attending.md) §13; [mission-deep-furrow.md](mission-deep-furrow.md) §13)
