...
export function separationSystem(world: SimWorld): void {
  ...
  for (let i = 0; i < units.length; i++) {
    ...
    const neighbours = grid.queryRadius(ax, ay, ra + MAX_UNIT_RADIUS_M, buffer);
    ...
    for (let j = 0; j < neighbours.length; j++) {
      ...
      if (d2 < SEPARATION.COINCIDENT_EPSILON_M * SEPARATION.COINCIDENT_EPSILON_M) {
        ...
        // is no axis to separate along, so pick one from the entity ids:
        // arbitrary, but identical on every machine that replays this tick.
        const angle = ((a * 2654435761 + b) % 628) / 100;
        nx = Math.cos(angle);
        ny = Math.sin(angle);
        overlap = minD;
      } else {
  ...
  for (let i = 0; i < units.length; i++) {
    ...
    for (let j = 0; j < structures.length; j++) {
      ...
      Position.x[a] = sx + (dx / d) * minD;
      Position.y[a] = sy + (dy / d) * minD;
    }
  }
}
