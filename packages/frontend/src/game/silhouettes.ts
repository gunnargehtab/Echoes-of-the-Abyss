...
export const HULL_LENGTH_M: Record<UnitKind, number> = {
  ...
  // The carriers and their craft (#838). A craft is 14-22 m, which is the
  // smallest thing the renderer has ever drawn as a hull — a third of a Light
  // Scout — and the length is a stat for the reason every other one is: the
  // simulation spaces them at half of it.
  [UnitKind.Gantry]: statsFor(UnitKind.Gantry).hullLengthM,
  [UnitKind.Rootstock]: statsFor(UnitKind.Rootstock).hullLengthM,
  [UnitKind.Succentor]: statsFor(UnitKind.Succentor).hullLengthM,
  [UnitKind.Offertory]: statsFor(UnitKind.Offertory).hullLengthM,
  [UnitKind.Spark]: statsFor(UnitKind.Spark).hullLengthM,
  [UnitKind.Runner]: statsFor(UnitKind.Runner).hullLengthM,
  [UnitKind.Treble]: statsFor(UnitKind.Treble).hullLengthM,
  [UnitKind.Versicle]: statsFor(UnitKind.Versicle).hullLengthM,
};
